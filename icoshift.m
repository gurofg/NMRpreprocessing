...
% 280314 (FrSa) Made it independent from Statistical toolbox + cosmetics
% 091014 (FrSa) Made it compatible ALSO with new Matlab realease 2014b (V 8.4.0) 
%% ERRORS CHECK
% Constant
...
if str2double(ver(1:4))<2014
    warning('The version of Matlab that you are using is old and this version of icoshift will work but might misbehave, please use the previous icoshift version (V 1.2) that you can download from www.models.life.ku.dk');
end
%% Check input
...
if  strcmpi(xT,'average')
    xT   = MeanwNaN(xP);
    note = 'average';
elseif strcmpi(xT,'median') %The median target has to be rewritten (real median spectrum and not column wise median + get rid of stat toolbox!)
    xT = nanmedian(xP);
    note = 'median';
...
if flag_coshift
   ...
   if  strcmpi(note,'average')
      xT = MeanwNaN(xP); %FRSA uses the new CO-shifted average as a target NANMEAN REPLACED!
   end
   if  strcmpi(xT,'median')
%       xT = nanmedian(xP); %FRSA uses the new CO-shifted median as a target NANMEDIAN TO BE REPALCED!
      xT = MeanwNaN(xP);
   end
   if  strcmpi(note,'average2')
      xT = MeanwNaN(xP); %FRSA uses the new CO-shifted average as a first iteration average target NANMEAN REPLACED!
   end
...
%            intervalnow = xCS(:,allint(i,2):allint(i,3)); % (FrSa) further align the previously aligned matrix
           ...
           if min_interv < 0; min_interv=0; end %How negative regions affect the average2
...
if options(1)==2
   ...
   bot       = min(xP(:));
   try
   sh        = gobjects(1,2); % FrSa compatible with 2014b
   catch
   end
   sh(sp2)     = subplot(sp1,1,sp2,'nextplot','add','ColorOrder',CM,'color','w');
   if flag_coshift
   ...
   title('Raw data');
   delete(ph)
try   set( sh(sp2),'ColorOrderIndex',1); %FRSA added to ripristinate CM order
catch      %FRSA compatibility with previous versions
end        %FRSA compatibility with previous versions
    if flag_coshift
      ph1 = plot(Scal(x),xPo(:,x)'); %line([mi ma; mi ma], [0 0; top top], 'Color', 'r', 'LineStyle', ':', 'LineWidth', 2)
      avrg = MeanwNaN(xPo); %NANMEAN REPLACED!
      clear xPo;
   ...
   if flag_coshift
      subplot(sp1,1,1)
      h = plot (Scal(x),avrg(x), 'Color','r', 'LineWidth', 1);
      legend (h, 'average');
      subplot(sp1,1,2)
      h = plot (Scal(x),xT(x), 'Color','r', 'LineWidth', 1);
      if avg2_flag %AVG2
   ...
   while auto == 1 % Automatic search for the best "n" for each interval
      ...
      for i_block = 1:nBlocks
         ...
         [dummy,ind(block_indices),Ri] = ... %FFT Co-Shifting
            CC_FFTShift(xT(1,refW),xP(block_indices,refW),[-n n 2 1 Filling]);  
            R = cat(1,R,Ri);
      end
      temp_index = -n:n;
      for i_sam = 1:nP
...
if flag_miss
   ...
   if ~ismatrix(X), error('Multidimensional handling of missing not implemented, yet'), end
   MissOff = NaN(1,mP);
   for i_signal = 1:mP
...
if isgraphics(h,'line') % FrSa compatible with 2014b
...
elseif isgraphics(h,'patch') % FrSa compatible with 2014b
...
% HISTORY
% 1.00.00 2009 Nov 24 -> Function derived from SumwNaN
...
% to be fixed: 
% 1) Median and NanMedian
% 2) Hilighted spectrum on top
% 3) warning for wrong selected scale (outside scale boundareis)
% 4) ...